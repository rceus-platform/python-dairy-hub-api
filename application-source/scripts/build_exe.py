...
def build_exe():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print("      dist/DairyHubAdminLogin.exe")
            print("\n[INFO] Tips:")
            print("   - You can copy the .exe to any Windows machine")
            print("   - No Python installation needed on target machine")
            print("   - Make sure the API server is accessible from that machine")
            print("   - Change API_BASE_URL in admin_login_ui.py if server is remote")
            print("\n" + "=" * 60)
        else:
